...
const collections: Record<string, {
  ...
  "handmade-spoon-rings": {
    ...
    description: "Our signature collection features vintage spoons meticulously heated, bent, and polished into stunning, one-of-a-kind rings. Each piece carries the history of its past life, transformed into a wearable story.",
  ...
  "unique-patterns": {
    ...
    description: "Explore the intricate details and unique floral patterns preserved from classic silverware. No two patterns are exactly alike, ensuring your ring is exclusively yours.",
  ...
  "choosing-the-story": {
    ...
    description: "The journey begins with you. Visit our workshop to browse our collection of raw, vintage silverware and select the exact piece that resonates with your personal style.",
  ...
  "customer-moments": {
    ...
    category: "Community",
    description: "See how our community wears their stories. From casual daily wear to special occasions, our handcrafted rings are designed to complement every individual's unique aesthetic.",
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!collection) {
    return { title: "Collection Not Found | 2Q Nhẫn Thuật" };
  }

  return {
    title: `${collection.title} - ${collection.category} | 2Q Nhẫn Thuật`,
    description: collection.description,
    openGraph: {
      title: `${collection.title} | 2Q Nhẫn Thuật`,
      description: collection.description,
      images: [
        {
...
export default async function WorksDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const collection = collections[slug];
    ...
    <main className="w-full relative min-h-screen bg-[var(--bg-default)] flex flex-col">
      ...
      <section className="relative pt-32 pb-16 px-[var(--gutter)] md:pt-48 md:pb-24 border-b border-[var(--border)]">
        <div className="absolute top-28 left-[var(--gutter)] md:top-36 md:left-[var(--gutter)] z-10">
          ...
          >
            <span className="transition-transform duration-300 group-hover:-translate-x-1">&larr;</span> back
          </Link>
        </div>
      ...
      <section className="py-20 md:py-32 px-[var(--gutter)] bg-[var(--bg-elevated)] text-center border-t border-[var(--border)]">
        <div className="max-w-2xl mx-auto flex flex-col items-center">
          ...
          <p className="font-body text-base md:text-lg text-[var(--text-muted)] mb-10 leading-relaxed">
            Inspired by this collection? Visit our workshop in Da Nang to choose your vintage spoon and watch it transform into a piece of wearable art.
          </p>
          <Link 
            href="/appointment" 
